...
def get_neighbors(
    pos: Tuple[int, int], grid: List[str], cheating: bool = False
) -> List[Tuple[int, int]]:
    ...
    for dy, dx in directions:
        ...
        if (
            ...
            and (cheating or grid[new_y][new_x] != "#")
        ):
...
def find_reachable_positions(
    grid: List[str], start: Tuple[int, int], max_steps: int
    ...
    while queue:
        ...
        for next_pos in get_neighbors(current, grid, cheating=True):
            ...
            if new_dist <= max_steps and (
                next_pos not in distances or new_dist < distances[next_pos]
            ):
...
def find_cheats(
    grid: List[str],
    normal_distances: Dict[Tuple[int, int], int],
    start: Tuple[int, int],
    end: Tuple[int, int],
) -> Dict[int, int]:
    MAX_CHEAT_LENGTH = 20
